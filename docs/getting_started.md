...
| `PROXY_CACHE_VALID_NOTFOUND`          | No        |                                                                                                                                                       | `1m`                                                | Sets caching time for response code 404. A cached error is shared by all clients and all URL-encoding variants of the path — see [Caching of Error Responses](#caching-of-error-responses)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `PROXY_CACHE_VALID_FORBIDDEN`         | No        |                                                                                                                                                       | `30s`                                               | Sets caching time for response code 403. A cached `403` is shared by all clients and all URL-encoding variants of the path, and is served as a sanitized `404` on every hit, so this value bounds how long a transient upstream auth failure (rotated credentials, clock skew) is served to all clients as confident-looking `404`s. Set `0s` to not cache `403`s at all. See [Caching of Error Responses](#caching-of-error-responses)                                                                                                                                                                                                                                                                                                                              |
...
object: every percent-encoding variant of a URL normalizes to the same key, and the viewer's `Host` header is not part
of the key, so whichever request reaches the origin first decides what every client sees for that object until the
entry expires.
...
holds — a previously cached `200` masks a new upstream failure until it expires (`PROXY_CACHE_VALID_OK`, default
`1h`). A cached error is held as a status-only entry in the cache's shared memory zone and never appears as a file
under `/var/cache/nginx/s3_proxy`, so an empty cache directory does not prove that no error is being served from the
cache.
...
Byte-range (sliced) requests cache only successful responses: the slice cache defines no validity for `403` or `404`,
so error responses to ranged requests are never cached.
...
which is routed to trailing-slash handling instead) and the equivalent `error_page` lines in
...
[s3-location-common-template]: https://github.com/nginxinc/nginx-s3-gateway/blob/master/common/etc/nginx/templates/gateway/s3_location_common.conf.template
...
Sanitization applies every time a response is delivered, including on cache hits: an upstream `403` cached for
`PROXY_CACHE_VALID_FORBIDDEN` is re-collapsed to the sanitized `404` on every hit until the cache entry expires, which
matters when debugging — see [Caching of Error Responses](#caching-of-error-responses).
