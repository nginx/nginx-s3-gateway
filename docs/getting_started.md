...
Boolean settings accept `true`, `yes`, and `1` to enable, and `false`, `no`, and `0` to disable, in any
letter case. Any other non-empty value fails startup with an error naming the variable. The table lists
the documented `true`/`false` forms.
...
| `AWS_EC2_METADATA_V1_DISABLED`        | No        | `true`, `false`                                                                                                                                       | `false`                                             | Flag disabling the IMDSv1 fallback for EC2 instance credentials, so that credential retrieval fails closed when an IMDSv2 token cannot be obtained. See [Running in EC2 with an IAM Policy](#running-in-ec2-with-an-iam-policy).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
`ALLOW_DIRECTORY_LIST` environment variable is set to `true`. Directory listing
...
When `PROVIDE_INDEX_PAGE` environment variable is set to `true`, the gateway will
